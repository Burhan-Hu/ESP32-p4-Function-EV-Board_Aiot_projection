...
void example_sensor_init(example_sensor_config_t *sensor_config, example_sensor_handle_t *out_sensor_handle)
{
    ...
    int enable_flag = 1;
    // Set sensor output stream, retry a few times in case sensor needs more time
    for (int retry = 0; retry < 5; retry++) {
        ...
        ESP_LOGW(TAG, "Start stream fail, retry %d/5, ret=0x%x", retry + 1, ret);
        vTaskDelay(pdMS_TO_TICKS(50));
    }
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Start stream fail after retries");
    }
    ESP_ERROR_CHECK(ret);
